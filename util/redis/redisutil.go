...
// 重写生成连接池方法
func newPool(redisIP string, db int, password string) *redis.Pool {

	return &redis.Pool{
		MaxIdle:   5,
		MaxActive: 20, // max number of connections
		Dial: func() (redis.Conn, error) {
			c, err := redis.Dial("tcp", redisIP, redis.DialDatabase(db), redis.DialPassword(password))
			return c, err
		},
	}
}
...
func GetRedisClient(address string, db int, password string) *RedisClient {
	...
	mapMutex.RUnlock()
	if !mok {
		redis = &RedisClient{Address: address, DB: db, Password: password, pool: newPool(address, db, password)}
		mapMutex.Lock()
